...
## Updated: 2024-09-13 by AMP for ionic strength column experiment
## 
# 1. Setup ---------------------------------------------------------------------
...
directory = "../tempest_ionic_strength/Data/Column Experiment Data/DOC"

Study_Code = "ColEx" # filter to samples only (pattern: TMP_ColEx_)
...
npoc_raw <- files %>% 
  map_df(read_data) %>% 
  filter(grepl(Study_Code, sample_name)) %>% # filter to samples only (pattern: TMP_ColEx_)
  bind_rows() 
...
curvepts <-files %>% 
  ...
  filter(grepl("STD_", sample_name) & !grepl("CKSTD_", sample_name)) %>% # filter to curves only
...
blanks <- blanks_raw %>% 
  ...
  summarize(npoc_blank= round(mean(npoc_raw[!is.na(npoc_raw)]), 2),
            npoc_blank_SD= round(sd(npoc_raw[!is.na(npoc_raw)]), 2),
...
metadata <- metadata.1 %>%
  mutate(
    Treatment = str_extract(Sample_name, "^[^.]+"),               # String before the first . is the treatment name
    Soil_Column = str_extract(Sample_name, "(?<=\\.)[^.]+(?=\\.)"),  # String between the two . is the soil column number
    Wash = str_extract(Sample_name, "(?<=\\.)[^.]+$")             # String after the last . is the wash number 
  ) %>%
  rename(sample_ID_short = Sample_name) %>%
  mutate(Wash = as.numeric(Wash))


npoc_meta <- npoc_flags %>%
  full_join(metadata, by= c("sample_name"= "Random_ID"))

ordered_npoc_meta <- npoc_meta[order(npoc_meta$Treatment, npoc_meta$Wash, npoc_meta$Soil_Column),]
