...
for (j in 1:nrow(subset_jags)) {
  ...
  lambda_DOC  <- subset_jags$lambda_DOC[j]
  ...
  for (c in 1:n_experiments) {
    ...
    # Calcium
    Ca_init <- exp(Ca_a * log(cond_flood[c]) + Ca_b)
    ...
    # Conductivity
    cond_inf <- exp(cond_a * log(cond_flood[c]) + cond_b)
    
    cond_mobile[1] <- (cond_flood[c] - cond_inf) + cond_init
    cond_pw[1] <- cond_mobile[1] + cond_inf
    
    # DOC pool
    DOC_pool <- DOC_init +
      (DOC_max * cond_flood[c] / (k_m + cond_flood[c]))
    ...
    DOC_R[1] <-
      DOC_pool / (1 + exp(-DOC_R_logit[1]))
    
    DOC_mobile[1] <- DOC_pool - DOC_R[1]
    ...
    if (n_t > 1) {
      ...
      for (i in 2:n_t) {
        ...
        # Ca
        Ca_mobile[i] <- Ca_mobile[i - 1] * lambda_Ca
        Ca_pw[i] <- Ca_mobile[i] + Ca_inf
        
        # Conductivity
        cond_mobile[i] <- cond_mobile[i - 1] * lambda_cond
        cond_pw[i] <- cond_mobile[i] + cond_inf
        ...
        DOC_R[i] <-
          DOC_pool / (1 + exp(-DOC_R_logit[i]))
        
        # Flux from resistant -> mobile
        DOC_R_moving[i] <-
          DOC_R[i - 1] - DOC_R[i]
        
        # Mobile DOC flushing
        DOC_mobile[i] <-
          (DOC_mobile[i - 1] + DOC_R_moving[i]) *
          lambda_DOC
        
        DOC_pw[i] <- DOC_mobile[i] + DOC_inf
...
all_experiments_simplest_long <- all_experiments_simplest %>% 
  select(Exp_Type, Treatment, treatment_id, conductivity_uSpercm, Ca_ppm, doc_mgperL, Timepoint) %>% 
